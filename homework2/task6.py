...
def robCircle(nums: List[int]) -> int:
    ...
    """
    return max(robLiner(nums[1:]), robLiner(nums[:-1]))


def robLiner(nums: List[int]) -> int:
    """Каждая ячейка - это максимальная сумма, которую мы можем украсть со всех домов до текущего, включая его."""
    dp = [0] * len(nums)
    dp[0] = nums[0]
